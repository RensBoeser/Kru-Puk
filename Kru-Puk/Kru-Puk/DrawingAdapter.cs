...
namespace Kru_Puk
{
  class DrawingAdapter
  {
    ...
    public void Draw(SpriteBatch spritebatch, Texture2D sprite, Point position, bool flipped, Color color)
    {
      if (flipped)
      {
        spritebatch.Draw(sprite, position.ToVector2(), null, color, 0.0f, new Vector2(0, 0), 1.0f, SpriteEffects.FlipHorizontally, 0.0f);
      }
      else
      {
        spritebatch.Draw(sprite, position.ToVector2(), null, color, 0.0f, new Vector2(0, 0), 1.0f, SpriteEffects.None, 0.0f);
      }
    }
